...
- [1. 特定検診・特定保健指導](#1-特定検診・特定保健指導)
    - [1.1. リスクとと保健指導の内容](#11-リスクとと保健指導の内容)
    - [1.2. 追加リスク判断基準](#12-追加リスク判断基準)
- [2. 糖尿病](#2-糖尿病)
    - [2.1. HbA1c（ヘモグロビンエーシーワン）](#21-hba1cヘモグロビンエーシーワン)
    - [2.2. 血糖とインスリン](#22-血糖とインスリン)
    - [2.3. インスリンが十分に働かない原因](#23-インスリンが十分に働かない原因)
- [3. oracleDBについて](#3-oracledbについて)
    - [3.1. ブロックサイズ](#31-ブロックサイズ)
    - [3.2. パラレル実行](#32-パラレル実行)
    - [3.3. インデックス](#33-インデックス)
        - [3.3.1. B-Tree](#331-b-tree)
        - [3.3.2. ROWIDとは](#332-rowidとは)
    - [3.4. DB用語](#34-db用語)
...
- 40~74歳の被保険者・被扶養者が対象
- メタボリックシンドロームの予防と改善が目的
- 特定検診結果に基づき必要度に応じた保健指導が行われる
- 特定保健指導：積極的支援と動機付け支援の二つがある
...
start
:特定検診を受診;
if (男性でかつ腹囲85cm以上 または 女性かつ腹囲90cm以上) then (yes)
    :肥満リスクA;
elseif (BMIが25以上) then (yes)
    :肥満リスクB;
else (いずれでもない)
    :メタボではない;
    end
...
②脂質
③血圧
④喫煙歴あり(上記のうち一つ以上該当の場合カウント);

if (追加リスク1個以上) then (yes)
    if (40~64歳ではない or 肥満リスクA or (肥満リスクB and 追加リスク1~2個)) then (yes)
        :支援内容:動機付け支援;
    else (no)
        :支援内容:積極的支援;
    endif
...
- 血糖：
  - 空腹時血糖値：100mg/dl以上　or
  - HbA1c：5.6%以上　or
  - 血糖に関する薬剤治療を受けている
- 脂質：
  - 中性脂肪値：150mg/dl以上　or
  - HDLコレステロール値：40mg/dl未満　or
  - 血中脂肪に関する薬剤治療を受けている
- 血圧：
  - 収縮期(最大)血圧：130mmHg以上　or
  - 拡張期(最小)血圧：85mmHg以上　or
  - 血圧に関する薬剤治療を受けている
...
参考：[B+木](https://ja.wikipedia.org/wiki/B%2B%E6%9C%A8)
受診勧奨
...
格納できる最大レコード数　n=b^h
最小キー数　2(b/2)^h-1
この機構造を格納するのに要する領域　O(n)
1つのレコードの挿入に要する操作回数は最悪で　O(logb n)
1つのれこーそを探すのに要する操作回数は最悪で　O(logb n)
位置がわかっているレコードの削除に要する操作回数は最悪で　O(logb n)
範囲クエリでk個の要素が見つかる場合に要する操作回数は最悪で　O(logb n+k)
```
